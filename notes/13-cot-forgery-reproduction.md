...
## Faithful result (vLLM, native MXFP4, tailored forgeries) — reproduced

Ran on the vLLM gpt-oss-20b server (native MXFP4; see notes/14) with per-prompt tailored
forgeries generated by gpt-oss itself (clean → prefill ladder, programmatic fallback), destyled
via the corrected content-preserving `destyle_text`. 313 prompts, ~40× faster than HF.

Forgery sources: 206 prefill, 45 clean, 62 programmatic (80% gpt-oss-authored; it refused the
rest even under prefill — those got the programmatic fallback).

**Refusal-heuristic compliance (pre-judge), against the paper and our template arm:**

| arm | faithful (tailored, MXFP4) | template (generic, bf16) | paper |
| --- | --- | --- | --- |
| baseline | 2.9% | 3% | 0–4% |
| **forgery** | **70.9%** | 32.3% | **60%** |
| destyled | **10.9%** | 0.3% | **10%** |

The tailored forgeries reproduce the paper's headline: baseline≈0 → forgery high → destyling
collapses it. **Destyled 10.9% matches the paper's 10% almost exactly.** The forgery heuristic
(71%) sits above the paper's judge-confirmed 60% because the refusal heuristic overcounts vs the
strict HARMFUL_RESPONSE judge — the judge-confirmed number (running) will trim it toward ~60%.

**Compliance by forgery source** — the cleanest evidence that *tailoring* is the lever:

| forgery source | forgery compliance |
| --- | --- |
| vllm_clean (gpt-oss authored, no prefill) | 84.4% |
| vllm_prefill (gpt-oss authored, forced) | 75.2% |
| programmatic (generic fallback) | 46.8% |

Tailored gpt-oss forgeries (75–84%) far exceed the generic programmatic ones (47%), which in
turn exceed the fixed template arm (32%). That ordering — tailored > programmatic-per-prompt >
fixed-template — is exactly what "the forgery's per-prompt interpretation drives ASR" predicts,
and it is why the earlier generic template undershot the paper. Judge-confirmed ASR pending.
